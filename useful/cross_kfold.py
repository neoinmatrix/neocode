...
data=(train-train.mean())/(train.max()-train.min())
data[8]=train[8]
...
for train_index, test_index in kf.split(data.index):
    X=data.ix[train_index].drop(8,axis=1).values
    y=data.ix[train_index][8].values
    clf.fit(X,y)
    X_test=data.ix[test_index].drop(8,axis=1).values
    predicted = clf.predict(X_test)
    # expected = data.ix[test_index][8].values
