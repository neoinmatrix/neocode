...
if __name__ == '__main__':
    ...
    df = pd.read_csv('autocreate.csv', header='infer',  sep=',', 
    ...
    U=pd.DataFrame.from_csv("./U.csv")
    U.columns=df['user_id'].unique()
    # print V.head()
    V=pd.DataFrame.from_csv("./V.csv",header=0)
    V.columns=df['item_id'].unique()
    # print V.head()
    # df.ix[0:19][""].values
    items=df.ix[:]["item_id"].values
    # print V[items]
    # print U[0]
    # print V[30]
    # exit()
    ...
    for item_id in items:
        print int(np.dot(V[item_id],U[0])+mean)," ",df['rating'][i] \
        ," ",df['user_id'][i]," ",df['item_id'][i]
        i+=1
        # break 

    # print len(df)
    print  math.sqrt(loss(df,U,V))/len(df)
    # /len(df)
