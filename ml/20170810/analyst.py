...
def get_sharp_angle(mouse,rx=30.0):
    x=mouse[0]
    y=mouse[1]
    t=mouse[2]
    xn=len(mouse[0])
...
def analyst(data):
    mouse=data[1]
    flag=0
    if get_sharp_angle(mouse,10)==True:
        flag+=1 
    if get_sharp_angle(mouse,20)==True:
        flag+=1 
    if get_sharp_angle(mouse,30)==True:
        flag+=1 
    return flag
...
def getitems(data):
    if data[0]==3 and np.random.randint(1000)==1:
        return True
    else:
        return False

output = rdd_all.map(splitx).filter(filterXNP).map(analyst)
# .filter(getitems).map(lambda d:d[1])
...
# output.saveAsTextFile(ouatput_path)