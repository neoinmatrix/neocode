...
# get features
vtrs={"sharp":[],"diff":[],"cxcy":[],"shape":[],"all":[]}
lbs={"sharp":[],"diff":[],"cxcy":[],"shape":[],"all":[]}
for d in trains:
    idx=d[0]
    label=d[3]
    f={"sharp":4,"diff":5,"cxcy":6,"shape":7,"all":8}
    for k,v in f.items():
        f[k]=d[v]
    ...
    if idx in range(2601):
        ...
        if idx in range(2901,3001):
            k="sharp"
        elif idx in range(2801,2901):
            k="diff"
        elif idx in range(2701,2801):
            k="cxcy"
        elif idx in range(2601,2701):
            k="shape"
        else:
            continue
        vtrs[k].append(f[k])
...
# training 
scalar={"sharp":[],"diff":[],"cxcy":[],"shape":[],"all":[]}
clf={"sharp":[],"diff":[],"cxcy":[],"shape":[],"all":[]}
for k in scalar:
...
output = rdd_all.map(splitx).filter(lambda d:get_xpn_mac(d[1])) \
