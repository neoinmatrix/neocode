...
def getfft(mouse):
    ...
    pft = PolynomialFeatures(degree = 8)
    pt = pft.fit_transform(t[:,np.newaxis])
    # ttp = poly_reg.fit_transform(tt[:,np.newaxis])
    lr = LinearRegression()
    lr.fit(pt, x[:,np.newaxis])
    # return lr.coef_[0]
    tt= np.linspace(0,t[-1],15)
    ptt = pft.fit_transform(tt[:,np.newaxis])
    sample=lr.predict(ptt).flatten()
    return np.fft.fft(sample)
