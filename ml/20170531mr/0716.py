...
def draw_analyst(idx,mouse,goal,label):
    pass
    notfind=[548,630,691,781]
    nmachine=[548,630,691,781]
    path='./data/16/notfind/'
    if idx in notfind:
        print idx
        if idx in nmachine:
            c='b'
...
def testResultAll(clf,scaler,pca,savepath='./data/0716tmp.txt',stop=1200,clf_extra='',scaler_extra=''):
    ds=dataset.DataSet()
    allnum=0
    mclass=[[],[],[],[],[],[],[],[]]
    rclass=[[],[],[],[],[],[],[],[]]
    ...
    for i in range(len(mclass)):
        ...
        with open("./data/16/m_%d.txt"%i,'w') as f:
            f.write(savestr)
    ...
    for i in range(len(rclass)):
        ...
        with open("./data/16/r_%d.txt"%i,'w') as f:
            f.write(savestr)
...
def maintest():
    ...
    testResultAll(clf,scaler,pca,savepath='./data/16/0716tmp.txt',stop=-1,clf_extra=clf2,scaler_extra=scaler2)
