...
def testResultAll(clf,scaler,pca,savepath='./data/0713tmp.txt',stop=1200):
    ...
    while True:
        ...
        if idx==False:
            break
        # if idx in [1,5,100,132,139,170,253,347,362,380,416,485,500]:
        #     # ax=plt.figure()
        #     # plt.plot(mouse[0],mouse[1])
        #     # plt.savefig('./data/notfind/3dx%d.png'%idx)
        #     fig = plt.figure()  
        #     ax = fig.add_subplot(111, projection='3d')
        #     if idx in [139,    253,            416,485,500]:
        #         c='b'
        #     else:
        #         c='g' 
        ...
        allnum+=1
    print "all:",len(mclass[0]),len(mclass[1]),len(mclass[2]),len(mclass[3]),len(mclass[4])
