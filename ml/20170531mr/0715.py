...
def getfeature2(idx,mouse,goal,label,use_all=False):
    ...
    tmp.extend(lastfive)
    # tmp.extend([mouse[2][0],mouse[2][-1]])
    # print len(tmp)
    # dv=get_derivative(mouse)
    # tmp.extend(dv)
    return np.array(tmp)

def testResultAll(clf,scaler,pca,savepath='./data/0713tmp.txt',stop=1200,clf_extra='',scaler_extra=''):
    ds=dataset.DataSet()
    allnum=0
    mclass=[[],[],[],[],[],[],[],[]]
    rclass=[[],[],[],[],[],[],[],[]]
    ...
    while True:
        ...
        # if idx in [423,457,468,477,483,484,485,490]:
        #     # ax=plt.figure()
        ...
        #     plt.title(str(idx))
        #     plt.savefig("./data/notfind/2d/y%d.png"%idx)
        #     plt.clf()
        # #     plt.close()
